...
export async function readProduct(raw: string) {
  ...
  for (let redirects = 0; redirects < 4; redirects++) {
    ...
    const answer = (await dns.json()) as {
      Answer?: { type: number; data: string }[];
    };
    const addresses = answer.Answer?.filter((a) => a.type === 1) || [];
    if (!addresses.length || addresses.some((a) => !publicAddress(a.data)))
      throw new Error('Please send a public product website URL.');
    const response = await fetch(url.href, {
      redirect: 'manual',
      headers: { 'User-Agent': 'CutProductReader/1.0', Accept: 'text/html' },
    ...
    const bytes = await readLimited(response, 700000);
    return {
      ...metadata(new TextDecoder().decode(bytes), url.href),
      url: url.href,
    };
  }
  throw new Error(
